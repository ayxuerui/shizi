...
- [ ] 2.6 Spike: test iOS Safari audio-unlock-on-gesture behavior and PWA storage eviction behavior; document constraints for the assessment build
...
- [ ] 2.8 Go/no-go checkpoint: confirm all licenses clear and both rendering spikes are acceptable before starting Section 3

## 3. Character data core

- [ ] 3.1 Assemble ~200-character candidate pool from frequency data + this project's curated selections
- [ ] 3.2 Confirm and record identity-set characters (薛, 亦, 霖, 小, 蓝, 莓) as non-productive
- [ ] 3.3 Collect parent hand-tagging pass: concreteness/imageability and pictographic-origin flags for all ~200 candidates
- [ ] 3.4 Integrate stroke-count and ordered stroke-path data per candidate character
- [ ] 3.5 Compute confusability relationships (shared-stroke / near-mirror / component-difference) across the pool
- [ ] 3.6 Build exclusion logic: candidates missing a required attribute are excluded from selection until supplied
- [ ] 3.7 Write the fixed 25-character Phase A sequence (grammar skeleton + identity + basic pictographs) as a reviewable, hand-authored list
- [ ] 3.8 License compliance: bundle `ARPHICPL.TXT` + a change-note with the subsetted stroke-path data, and add `data/ATTRIBUTIONS.md` covering CC-CEDICT's BY-SA attribution (per `data/PROVENANCE.md` action items)

## 4. Learner state

- [ ] 4.1 Define the event schema (id, timestamp, session id, character, activity/modality, outcome, latency, position-in-session, prior-exposure-count, days-since-last-exposure, time-of-day, adult-present)
- [ ] 4.2 Implement append-only event log with client-generated idempotency keys
- [ ] 4.3 Implement mastery-state projection (`unseen`/`probing`/`known`/`shaky`) per the promotion/demotion rules
- [ ] 4.4 Implement known-set projection derived from mastery state
- [ ] 4.5 Implement rejection of malformed/incomplete events and of any mutate/delete attempt on existing events
- [ ] 4.6 Implement repo-side JSONL export script (hosted store → durable versioned file)
- [ ] 4.7 Write projection replay tests: given a fixed event log, projections are deterministic and reproducible after logic changes

## 5. Content validator

- [ ] 5.1 Implement whitelist check (identity ∪ known ∪ declared new-targets) with hard-failure reporting
- [ ] 5.2 Implement new-target repetition-threshold check (default minimum 8) with hard-failure reporting
- [ ] 5.3 Implement new-character density check (default max 5%) with hard-failure reporting
- [ ] 5.4 Implement shaky-character seeding advisory check (warning only)
- [ ] 5.5 Implement confusable-adjacency advisory check (warning only)
- [ ] 5.6 Implement structured result type distinguishing hard failures from warnings, with rule id and location per finding
- [ ] 5.7 Unit tests covering each rule's pass/fail/warn boundary conditions

## 6. Curriculum sequencer

- [ ] 6.1 Implement Phase A selection: draw next not-yet-known character from the fixed sequence, skipping already-known ones
- [ ] 6.2 Implement Phase A exhaustion detection (transition point to Phase B)
- [ ] 6.3 Implement Phase B scoring function (word-unlock, story-unlock, personal relevance, learnability, confusability penalty) with configurable weights
- [ ] 6.4 Implement confusability hard-constraint filter against recently-introduced characters
- [ ] 6.5 Implement "no eligible candidate" fallback (decline to select rather than violate spacing)
- [ ] 6.6 Determinism tests: same state + config → same selection

## 7. Adaptivity instrumentation

- [ ] 7.1 Ensure all activity event-writers populate full schema, including fields with no current consumer (time-of-day, adult-present, session-position)
- [ ] 7.2 Implement matched-pair identification (stroke count, concreteness, frequency, confusability-neighborhood size)
- [ ] 7.3 Implement random arm assignment for matched pairs, recorded independent of outcome
- [ ] 7.4 Implement parent one-tap end-of-session rating capture (loved it / fine / checked out), linked to session id, with skip-without-blocking behavior
- [ ] 7.5 Confirm no ranking/effectiveness/routing computation exists anywhere in this change's code (explicit review checklist item)
