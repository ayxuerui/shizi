...
### Section 9: Cloudflare Pages/Worker/D1 → self-hosted (tunnel only)

The "Architecture overview" section above specified Cloudflare Pages (static hosting) + Worker (sync endpoint) + D1 (event store). Building Section 9 surfaced that this project's dev environment has no authenticated Cloudflare account at all (`wrangler whoami` confirms it) — and, separately, that the repo already has a proven pattern for exactly this situation: `docker-compose.yml`'s `spikes` service already serves content through the existing `cloudflared-net` tunnel to a Cloudflare Zero Trust hostname, entirely without Cloudflare API access. **User decision this session:** self-host everything (static files, sync endpoint, event store) on the same machine, extending that pattern rather than provisioning new cloud resources. Cloudflare's role shrinks to "tunnel only" — the Zero Trust ingress rule for any new hostname is the user's own dashboard action, unchanged from how `spikes` already works.

Concretely: `infra/sync-service` replaces the Worker (plain Node/TypeScript, no Express — one real route each doesn't need a router; request-handling logic in `handle-sync.ts` is deliberately written with no Node `http` types in its signature, so a real Worker migration later would need only a thin adapter, not a rewrite) and `better-sqlite3` replaces D1 (the same underlying engine D1 itself uses, so the schema/idempotency approach — `INSERT OR IGNORE` keyed on `LearnerEvent.id`, the same idempotency key the client's own `EventLog` already uses — would carry over unchanged if this ever did move to a real D1 instance). `infra/nginx-assessment.conf` + a new `gateway` docker-compose service (container name `shizi-gateway`) replace Pages, reusing `spikes/nginx-default.conf`'s pattern verbatim plus a `Cache-Control: no-cache` rule on `sw.js` (a cached service-worker file would permanently block PWA updates — a real correctness issue, not just a nicety).

**Found and fixed during verification, not just assumed:** `better-sqlite3@13.0.3` requires Node ≥22 — installing it on a Node 20 image produces only an `EBADENGINE` *warning*, not a build failure, but the resulting native binary segfaults (exit code 139) the moment it's used. Caught by actually running the built image, not just reading the warning; `infra/sync-service`'s Dockerfile and `package.json`'s `engines` field both pin Node 22 as a result.

**Backup plan (the user asked directly, answered here, not with another question):** design.md's own already-stated principle — the repo-side JSONL export is the actual canonical, durable record; losing the hosted store is an inconvenience, not data loss — applies unchanged to self-hosted SQLite. Primary backup is `scripts/pull-events.ts` regenerating `data/events/events.jsonl`, committed to git (every remote/clone is then a full off-machine copy). Secondary, cheap insurance: the live file sits on a named Docker volume (survives container recreation) plus a periodic `better-sqlite3` `.backup()` snapshot (`src/backup.ts`, every 6 hours, keeps the last 14) to a separate timestamped file on the same volume — protects against corruption between JSONL pulls, not a disk failure. **Flagged, not solved:** true off-site/disk-failure-surviving backup needs external storage credentials nobody has supplied — an open item, not invented unasked-for.

**Trade-off accepted:** self-hosting means the user's own machine's uptime is now this project's uptime, where Cloudflare Pages/D1 would have offered managed availability. Accepted as clearly the right trade for a single-family, low-stakes tool with no Cloudflare account to build on anyway — see the "Auth: shared token, not accounts" decision above for the same scale-appropriate reasoning.

**Domain reuse, decided directly by the user:** `shizi.realxco.com` was already tunneled to `spikes` — rather than provisioning a second hostname for the real app, that same domain gets repointed at the `gateway` container (a one-line Zero Trust dashboard edit, no new ingress rule). Its nginx reverse-proxies `/sync/*` to the `sync` container internally (`infra/nginx-assessment.conf`), so the app and its sync endpoint end up same-origin — `VITE_SYNC_ENDPOINT` is just the relative path `/sync`, with no CORS configuration needed at all. `spikes` keeps running for local-only access; it's just no longer what the public hostname resolves to.

**Single holding service, path-based routing — also decided directly by the user:** *"how about shizi.realxco.com/assessment? So that I only need to manage one domain and one holding service."* The app moves from the domain root to `shizi.realxco.com/assessment/`, so this one nginx container remains the ONE tunnel target indefinitely — a future second app would get its own path prefix on this same container rather than a second Zero Trust ingress rule. Per the same conversation, the container itself is named `shizi-gateway` (`docker-compose.yml`'s `gateway` service), not `shizi-assessment` — it isn't just the assessment app's container, it's the one thing the tunnel needs to point at regardless of what else ends up served from it. This is a real build change, not just a routing/naming tweak: `vite.config.ts` sets `base: "/assessment/"` (dev/preview now match prod exactly rather than diverging), the PWA manifest's `start_url`/`scope`/icon paths are relative so they resolve under wherever the manifest itself is served from, every hand-written absolute-path reference in app code (`fetch("/config.json")`, the two `<audio src>` paths) now reads `import.meta.env.BASE_URL` instead of a hardcoded `/`, and `fonts.css`'s `url()` is hardcoded to `/assessment/fonts/...` since Vite does not rewrite absolute `public/`-referenced paths in hand-authored CSS when `base` isn't `/`. `nginx-assessment.conf` uses `alias` (not `root`) to strip the `/assessment/` prefix before the on-disk file lookup, nests the sync reverse-proxy at `/assessment/sync/` (keeping this app's whole footprint — static files and its API — under one self-contained prefix rather than a sibling top-level `/sync`), and redirects both the bare domain root and the bare `/assessment` path (no trailing slash — nginx's prefix-location matching requires it literally, and a bare-path request otherwise matches no location block and 404s) to `/assessment/`. Verified via a real `docker compose up` + `curl` pass, including against the actual public hostname: static assets, manifest MIME type, `sw.js` cache header, the font asset, and an authenticated `/assessment/sync/events` POST all round-tripped correctly. **Found via the user's own real-browser test, not caught by local curl alone:** the public hostname was still pointed at the long-running, separately-started `spikes` container, not `gateway` — a dashboard repoint the user still needs to make (flagged in `infra/README.md`, not something this repo can do).

### Section 10 prep: parent-rating durable storage schema

Task 7.4 originally shipped the one-tap rating UI with persistence flagged as a follow-up, since `learner-state`'s `EventLog`/`LearnerEvent` schema requires `character`, `outcome`, and `latencyMs` — fields a session-level rating genuinely has none of, and inventing sentinel values for them would be worse than leaving the gap flagged. **Decision:** `SessionRating` is a sibling record type in `@shizi/adaptivity`, not `learner-state` — the `adaptivity-instrumentation` spec capability that actually states the "record it as an event associated with that session" requirement, matching where `ArmAssignment` (another non-character-keyed instrumentation record) already lives. `sessionId` is the primary/idempotency key: the spec calls for "exactly one simple rating prompt," and the UI's `ratingPhase` reducer settles after one tap, so a second rating for the same session is unreachable by construction — `INSERT OR IGNORE` on it is a safe idempotent re-sync, the same guarantee events already get from their own natural key. Follows the client's `events` queue pattern (natural key) rather than `assignments`' surrogate-key pattern, since a rating — unlike an assignment — has one.

**Deliberately not built:** a free-text notes field for task 10.3's "qualitative observations." The spec's one-tap requirement and a text input are in tension, a keyboard mid-session is the wrong UX for this age group, and any new Chinese label would need a font-subset rebuild. Left as a to-be-decided out-of-band note-taking convention, not invented unasked-for.

### Section 10 prep: on-device diagnostics screen (task 10.0)

Task 10.0's pre-flight checklist was a purely manual walkthrough, and its biggest named risk — zh-CN `SpeechSynthesis` availability on iPad Safari — had a dead mitigation seam: `narration.ts`'s `isAvailable()` only checked that the `SpeechSynthesis` API object existed, which is true on iPad even with zero Chinese voices installed, i.e. true in exactly the failure case it exists to catch. **Decision:** build `apps/assessment/src/diagnostics/`, an in-app pre-flight surface, and fix that seam as part of doing it — shipping a screen whose whole purpose is that risk while leaving the seam dead would be incoherent.

**Entry mechanism:** a 1.5s long-press on an unlabeled corner of the unlock screen is the primary path, plus a `#diagnostics` hash kept only as a faster dev/Safari convenience. A `VITE_*` env flag was rejected as the entry mechanism — toggling it needs a rebuild+redeploy, meaning the build being diagnosed would never be the build actually shipped. The corner long-press is what makes the checklist's airplane-mode/standalone item checkable at all: it renders on the one screen every cold start (including an offline one) always shows, inside standalone/home-screen mode where there's no address bar for a hash. A local storage flag bridging Safari to the installed home-screen app was considered and rejected — iOS gives a home-screen web app a separate storage container from the browser tab, so a flag set in Safari is not visible to the installed app.

**Machine evidence vs. human verdict, kept structurally separate:** every diagnostics check produces a `CheckResult` (proof a thing fired — a voice object exists, `onstart`/`onend` fired, `currentTime` advanced) and, for anything only a human sense can judge, a separately-recorded `HumanVerdict` ("I heard it" / "it did not work"). The machine never infers the latter from the former — `speakAndObserve` proves synthesis fired, not that it was audible or intelligible; `probeElementPlayback` proves playback progressed, not that it was heard. `PenPalmProbe.tsx` observes the real app-wide `pointerGate` singleton's actual decisions (a new `subscribe` seam on `PointerGate`, additive, no behavior change to the real tap path) rather than a simulated one — whether palm rejection holds with a real Pencil on a real device remains a human judgment this component can only make visible, not confirm.

**Never reachable from the child's session:** `App.tsx` renders EITHER `DiagnosticsScreen` OR `AudioUnlockGate`/`BoutScreen`, never both, which is what keeps `BoutScreen.test.tsx`'s `assertNoScoreLikeText` guarantee (no digit anywhere in `document.body.textContent`) fully intact — this screen's digits (byte counts, voice counts, timestamps) never enter that tree. Its pass/fail affordance (`OK`/`ATTENTION`/`UNKNOWN`) is a module-local palette in `diagnostics/theme.ts`, deliberately NOT a new `--color-error` token in `styles/tokens.css` — that file's absence of one is itself the enforcement mechanism behind "no visible scoring or failure state," and this screen is parent-facing, pre-flight tooling, never reachable by the child. All labels are English/ASCII — this whole module is parent-facing, and any Chinese text would need a font-subset rebuild for a screen the child never sees.
...
- **[Trade-off]** Self-hosting Section 9 (instead of Cloudflare Pages/Worker/D1) means this project's uptime is now tied to the user's own machine, not a managed cloud service → accepted, since there's no Cloudflare account to build on anyway and this is single-family, low-stakes data (same reasoning as "Auth: shared token, not accounts"). **Mitigation for the one real durability concern:** the repo-side JSONL export remains the canonical backup regardless of where the hosted event store lives.
