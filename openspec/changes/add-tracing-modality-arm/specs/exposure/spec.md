...
Introduces a not-yet-known character to the learner through an arm-assigned teaching activity, so the
matched-pair randomization the adaptivity-instrumentation capability performs has something real to
route between and Loop 3 can eventually compare teaching-activity effectiveness. (Vocabulary note:
events record `module: "learn"` and the delivered `activity` — `listen` or `trace` — per
`rename-event-modality-to-activity`, which retired the `expose-*` modality identifiers this delta
...
Exposure SHALL deliver the teaching activity recorded in the selected character's arm assignment. If no
...
- **THEN** exposure SHALL assign and record an arm for it before presenting any activity-specific content
  to the learner

### Requirement: Guided tracing only
The tracing exposure activity SHALL keep a visible stroke-order template in place for the full duration
...
- **WHEN** a learner is using the tracing activity to be introduced to a character
...
schema) carrying `module: "learn"` and the delivered arm's activity (`listen` or `trace`). That
activity SHALL NOT be a member of the recognition-activity set the learner-state capability's
mastery projection reads from.

#### Scenario: Exposure event logged with its module and activity
- **WHEN** a learner completes an exposure interaction
- **THEN** the system SHALL log one event whose `module` is `"learn"` and whose `activity` field
  identifies the arm actually delivered (`listen` or `trace`)
...
Exposure SHALL be fully usable with no network connectivity, including any activity-specific audio or
