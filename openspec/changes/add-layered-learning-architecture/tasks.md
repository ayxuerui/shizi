## 1. Progression layer: the progress context projection

- [ ] 1.1 Define `ProgressContext` in `packages/learner-state/src/progress-context.ts`: mastery state per unit, known set, set of units ever presented, last-exposure timestamp per unit, and first-introduction ordering (oldest first)
- [ ] 1.2 Implement `deriveProgressContext(events, config)` as a pure projection over the event log, computing "ever presented" from all events and mastery from recognition-modality events only (design decision 3 — these are different event subsets and must not be conflated)
- [ ] 1.3 Export the type and function from `packages/learner-state/src/index.ts`
- [ ] 1.4 Tests in `progress-context.test.ts`: an introduction-only unit reports as presented with no mastery state; a unit with two fast-correct recognition events reports mastered; introduction order follows first-ever exposure across all modalities; last-exposure follows the most recent event of any modality; two calls against the same log return identical facts; an empty log yields an empty context
- [ ] 1.5 Confirm no existing `learner-state` test changed and the whole package suite still passes
...
- [ ] 2.2 Add a batch-request entry point accepting a `ProgressContext` (rather than the current hand-assembled `CurriculumState`) and returning a batch of `LearningGoal`s, delegating to the existing `composeBatch` — no new selection logic
...
- [ ] 2.5 Tests: a batch derived from a `ProgressContext` matches the equivalent `composeBatch` call; each goal carries its kind; a short batch is returned rather than a spacing violation; batch composition is reproducible across two identical calls
...
- [ ] 3.4 Port the app's existing `computeDueForMemory` recency rule into a clearly-labelled **placeholder** queue provider, reading a `ProgressContext`, to stand in until `review-scheduling` is implemented — named and documented so it cannot be mistaken for the real retention model
...
- [ ] 4.3 Implement `nextActivity(context, plan, orchestrationState, config)` as a pure function, porting the decision logic from `apps/assessment/src/session/activity-selector.ts` — reading the progress context rather than re-deriving mastery, recency, or introduction order
- [ ] 4.4 Implement the batch-open rule: filter the next published batch against the current progress context, then freeze the resulting concrete goal list into orchestration state; identify batches by content, never by index (design decision 5)
...
- [ ] 6.1 Reduce `apps/assessment/src/session/PracticeRouter.tsx` to: load the progress context and published plan, ask the learning layer for a decision, render the matching activity screen, and call `reportActivityOutcome` on completion
